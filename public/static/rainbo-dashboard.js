        ...
        <main class="max-w-7xl mx-auto py-6 px-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="text-xs text-gray-500 mt-1">Active cases</div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="text-3xl font-bold text-red-600 mt-2" id="urgent-cases">0</div>
        ...
        if (data.cases && data.cases.length > 0) {
            ...
            document.getElementById('cases-container').innerHTML = `
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-sm mt-2">You will receive notifications when new cases are assigned to Rainbo Centre</p>
                </div>
            `;
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ${cases.map(c => `
                        <tr class="hover:bg-gray-50 ${c.unread_count > 0 ? 'bg-blue-50' : ''}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getPriorityColor(c.assignment_priority)}">
    ...
    try {
        const response = await fetch(`/api/assignments/${assignmentId}/status`, {
            ...
            body: JSON.stringify({
                ...
                notes: 'Case accepted by Rainbo Centre staff'
            })
        });
        ...
        alert('Case accepted successfully! You can now begin medical services.');
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
            ...
            <div class="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600">Assigned to Rainbo Centre</p>
                </div>
            ...
            <div class="p-6">
                ...
                <!-- Add Medical Service Form -->
                <div class="border-t pt-6">
                    <h3 class="font-semibold text-gray-900 mb-4">
                        <i class="fas fa-notes-medical mr-2"></i>Record Medical Service
                    </h3>
                    <form onsubmit="submitMedicalService(event, ${caseId}, '${caseInfo.case_number}')" class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Examination Type</label>
                                <select name="examination_type" required class="w-full border rounded px-3 py-2">
                                    <option value="">Select...</option>
                                    <option value="initial_assessment">Initial Assessment</option>
                                    <option value="forensic_exam">Forensic Examination</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Services Provided</label>
                                <input type="text" name="services_provided" required placeholder="e.g., Medical exam, counseling" class="w-full border rounded px-3 py-2">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="flex items-center">
                                    <input type="checkbox" name="pep_administered" class="mr-2">
                                    <span class="text-sm font-medium text-gray-700">PEP Administered</span>
                                </label>
                            </div>
                            <div>
                                <label class="flex items-center">
                                    <input type="checkbox" name="sti_testing_done" class="mr-2">
                                    <span class="text-sm font-medium text-gray-700">STI Testing Done</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="flex items-center">
                                    <input type="checkbox" name="injuries_documented" class="mr-2">
                                    <span class="text-sm font-medium text-gray-700">Injuries Documented</span>
                                </label>
                            </div>
                            <div>
                                <label class="flex items-center">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Treatment Provided</label>
                            <textarea name="treatment_provided" rows="2" placeholder="Description of treatment" class="w-full border rounded px-3 py-2"></textarea>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Additional Notes</label>
                            <textarea name="notes" rows="3" placeholder="Any additional observations or notes" class="w-full border rounded px-3 py-2"></textarea>
                        </div>
                        ...
                        <div class="flex justify-end space-x-3">
                            ...
                            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                                <i class="fas fa-save mr-2"></i>Save Medical Service
                            </button>
                        </div>
                    </form>
                </div>
...
// Submit medical service
async function submitMedicalService(event, caseId, caseNumber) {
    ...
    const data = {
        user_id: sessionData.id,
        examination_type: formData.get('examination_type'),
        services_provided: formData.get('services_provided'),
        pep_administered: formData.get('pep_administered') === 'on',
        sti_testing_done: formData.get('sti_testing_done') === 'on',
        injuries_documented: formData.get('injuries_documented') === 'on',
        treatment_provided: formData.get('treatment_provided') || null,
    ...
    try {
        ...
        alert('Medical service recorded successfully!');
        ...
        alert('Failed to record medical service. Please try again.');
    }
}
...
function viewCase(caseNumber) {
    ...
    fetch('/api/organization/rainbo/cases')
        .then(r => r.json())
        .then(data => {
            const caseObj = data.cases.find(c => c.case_number === caseNumber);
            if (caseObj) {
